...
struct group_member
{
	...
	int con_fd;       //初始化为-1，为不在线，群发消息时看是否在线
};
...
struct Info
{
	...
	int exist;         //是否存在，存在为0，不存在为-1
	int con_fd;       //通信用的套接字每个人一个。
	int flag;        //是否在线，在线为0， 不在线为-1
...
static struct Info user[MAX_USER];
static struct group_info group[5];//5个群
